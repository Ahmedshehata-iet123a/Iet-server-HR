...
class RequestOfSpecifyLetter(models.Model):
    _name = 'request.specify.letter'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'name'

    sequence = fields.Char(default=lambda self: _('New'),
                           copy=False, readonly=True, tracking=True, string='Request Number')
    name = fields.Char(string='Request Name', default='Request for a letter of introduction')
        ...
        for vals in vals_list:
            if vals.get('sequence', _('New')) == _('New'):
                vals['sequence'] = (self.env['ir.sequence'].
                                    next_by_code('request.specify.letter.seq.code'))
        ...
        for user in users:
            self.env['mail.activity'].create({
                ...
                'res_model_id': self.env['ir.model'].search(
                    [('model', '=', 'request.specify.letter')]).id,
                'user_id': user.id,
            })
    ...
    def action_submit(self):
        for rec in self:
            ...
            model_id = self.env['request.specify.letter'].search([('id', '=', rec.id)])
