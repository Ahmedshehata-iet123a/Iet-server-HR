...
class RequestTransferEmployee(models.Model):
    _name = 'request.transfer.employee'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'name'

    sequence = fields.Char(default=lambda self: _('New'),
                           copy=False, readonly=True, tracking=True, string='Request Number')
    name = fields.Char(string='Request Name', default='Employee transfer request')
    ...
    current_branch = fields.Many2one('location', string='Current Location', related='employee_id.location',
                                     tracking=True,
                                     store=True)
    old_branch = fields.Many2one('location', string='Old Location', readonly=True)
    new_branch = fields.Many2one('location', string='New Location', )
        ...
        for vals in vals_list:
            if vals.get('sequence', _('New')) == _('New'):
                vals['sequence'] = (self.env['ir.sequence'].
                                    next_by_code('request.transfer.employee.seq.code'))
        ...
        for user in users:
            self.env['mail.activity'].create({
                ...
                'res_model_id': self.env['ir.model'].search(
                    [('model', '=', 'request.transfer.employee')]).id,
                'user_id': user.id,
            })
    ...
    def action_submit(self):
        for rec in self:
            ...
            model_id = self.env['request.transfer.employee'].search([('id', '=', rec.id)])
    ...
    def action_hr_approval(self):
        for rec in self:
            old_location = rec.employee_id.location
            if rec.new_branch:
                rec.employee_id.location = rec.new_branch
            rec.old_branch = old_location
            rec.state = 'done'
