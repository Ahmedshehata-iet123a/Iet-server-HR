...
class RequestOfficeSupplies(models.Model):
    _name = 'request.office.supplies'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'name'

    sequence = fields.Char(default=lambda self: _('New'),
                           copy=False, readonly=True, tracking=True, string='Request Number')
    name = fields.Char(string='Request Name', default='Office supplies request')
        ...
        for vals in vals_list:
            if vals.get('sequence', _('New')) == _('New'):
                vals['sequence'] = (self.env['ir.sequence'].
                                    next_by_code('request.office.supplies.seq.code'))
        ...
        for user in users:
            self.env['mail.activity'].create({
                ...
                'res_model_id': self.env['ir.model'].search(
                    [('model', '=', 'request.office.supplies')]).id,
                'user_id': user.id,
            })
    ...
    def action_submit(self):
        for rec in self:
            ...
            model_id = self.env['request.office.supplies'].search([('id', '=', rec.id)])
