...
class RequestPlumbingAndElectrical(models.Model):
    _name = 'request.plumbing.and.electrical'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'name'

    sequence = fields.Char(default=lambda self: _('New'),
                           copy=False, readonly=True, tracking=True, string='Request Number')
    name = fields.Char(string='Request Name', default='Request for plumbing and electrical maintenance')
    ...
    # request_to = fields.Char(string='موجه إلي', tracking=True)
        ...
        for vals in vals_list:
            if vals.get('sequence', _('New')) == _('New'):
                vals['sequence'] = (self.env['ir.sequence'].
                                    next_by_code('request.plumbing.and.electrical.seq.code'))
        ...
        for user in users:
            self.env['mail.activity'].create({
                ...
                'res_model_id': self.env['ir.model'].search(
                    [('model', '=', 'request.plumbing.and.electrical')]).id,
                'user_id': user.id,
            })
    ...
    def action_submit(self):
        for rec in self:
            ...
            model_id = self.env['request.plumbing.and.electrical'].search([('id', '=', rec.id)])
